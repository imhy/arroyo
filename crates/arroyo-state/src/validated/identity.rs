...
//!
//! Every whole object in [`crate::validated`] is *about* a particular checkpoint of a
//! particular job, and every one of them is assembled out of values that arrive
//! independently: a job id and an epoch the caller asked for, a top-level metadata object
//! read back out of storage, one persisted operator object per operator, and — for a
//! checkpoint this process is taking — a report per subtask. Each of those carries its own
//! copy of the identity. Nothing makes the copies agree.
//!
...
//!   `OperatorMetadata.job_id`/`operator_id`/`epoch` — is input from storage, and after
//!   M11.T25d it is authoritative input: a token minted over it entitles effects, and
//!   expiring-table compaction builds its output paths from it
...
/// One checkpoint of one job: the identity every token in [`crate::validated`] is about.
///
...
impl CheckpointIdentity {
    ...
    /// [`arroyo_rpc::state_backend::validated::WholeObject`].
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    use arroyo_rpc::grpc::rpc::{OperatorCheckpointMetadata, OperatorMetadata};
